...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        private void ShowTrackFlyout()
        {
            flp1.Visible = true;

            int tracksVisible = 0;
            bool hasABLine = false;
            bool isBnd = bnd.bndList.Count > 0;

            for (int i = 0; i < trk.gArr.Count; i++)
            {
                if (trk.gArr[i].isVisible) tracksVisible++;
                if (trk.gArr[i].mode == TrackMode.AB) hasABLine = true;
            }
            ...
            //standard AB draw button
            flp1.Controls[1].Visible = isBnd && !isEasyDriveMode;

            //auto AB from boundary is only shown before the first AB line exists
            flp1.Controls[2].Visible = isBnd && !hasABLine && !isEasyDriveMode;

            flp1.Controls[3].Visible = true;
            flp1.Controls[4].Visible = true;
            ...
            //ref nudge
            flp1.Controls[7].Visible = tracksVisible > 0;
            ...
            flp1.Top = Height - 120 - (btnCount * 75);
            flp1.Left = Width - 120 - flp1.Width;
            trackMethodPanelCounter = 4;
        }
        private void btnAutoSteer_Click(object sender, EventArgs e)
        {
            longAvgPivDistance = 0;
            bool wasAutoSteerOn = isBtnAutoSteerOn;
            ...
            if (!timerSim.Enabled)
            {
                if (avgSpeed > vehicle.maxSteerSpeed)
                {
                    if (isBtnAutoSteerOn)
                    {
                        ...
                        SyncSteerWorkSwitchManualSectionsWithAutoSteer();
                    }
            ...
            if (wasAutoSteerOn != isBtnAutoSteerOn)
            {
                SyncSteerWorkSwitchManualSectionsWithAutoSteer();
            }
        }
        ...
        private void btnAutoABFromBoundary_Click(object sender, EventArgs e)
        {
            if (bnd.bndList.Count == 0)
            {
                TimedMessageBox(2000, gStr.gsNoBoundary, gStr.gsCreateABoundaryFirst);
            ...
            int created = CreateABLinesFromBoundarySides();
            if (created == 0)
            {
                TimedMessageBox(2000, "Boundary AB", "No boundary side longer than 5 m.");
                return;
            }

            TimedMessageBox(2000, "Boundary AB", "Created " + created.ToString(CultureInfo.CurrentCulture)
                + " AB line(s) from boundary.");

            ShowTrackFlyout();
            PanelUpdateRightAndBottom();
        }

        private int CreateABLinesFromBoundarySides()
        {
            ...
            {
                List<Tuple<vec3, vec3>> boundarySides = BuildBoundarySides(bnd.bndList[bndIndex].fenceLine);

                for (int i = 0; i < boundarySides.Count; i++)
                {
                    vec3 pointA = boundarySides[i].Item1;
                    vec3 pointB = boundarySides[i].Item2;

                    double deltaEast = pointB.easting - pointA.easting;
                    double deltaNorth = pointB.northing - pointA.northing;
                    double length = Math.Sqrt(deltaEast * deltaEast + deltaNorth * deltaNorth);
                    if (length < minSegmentLengthMeters) continue;

                    double heading = Math.Atan2(deltaEast, deltaNorth);
                    ...
                    trk.gArr.Add(new CTrk
                    {
                        mode = TrackMode.AB,
                        isVisible = true,
                        ptA = new vec2(pointA.easting, pointA.northing),
                        ptB = new vec2(pointB.easting, pointB.northing),
                        heading = heading,
                        nudgeDistance = 0,
                        name = "Bnd " + (bndIndex + 1).ToString(CultureInfo.InvariantCulture)
                            + "-" + (created + 1).ToString(CultureInfo.InvariantCulture)
        ...
        private List<Tuple<vec3, vec3>> BuildBoundarySides(List<vec3> fenceLine)
        {
            const double tinySegmentMeters = 0.25;
            const double sameSideAngleTolerance = 20.0 * Math.PI / 180.0;

            List<vec3> points = CleanBoundaryPoints(fenceLine);
            List<Tuple<vec3, vec3>> sides = new List<Tuple<vec3, vec3>>();
            int pointCount = points.Count;
            if (pointCount < 2) return sides;

            List<vec3> segmentStarts = new List<vec3>();
            List<vec3> segmentEnds = new List<vec3>();
            List<double> headings = new List<double>();

            for (int i = 0; i < pointCount; i++)
            {
                vec3 pointA = points[i];
                vec3 pointB = points[(i + 1) % pointCount];
                double length = BoundaryPointDistance(pointA, pointB);
                if (length < tinySegmentMeters) continue;

                double heading = Math.Atan2(pointB.easting - pointA.easting, pointB.northing - pointA.northing);
                if (heading < 0) heading += glm.twoPI;

                segmentStarts.Add(pointA);
                segmentEnds.Add(pointB);
                headings.Add(heading);
            }

            int segmentCount = headings.Count;
            if (segmentCount == 0) return sides;

            int startSegment = 0;
            double largestTurn = -1;
            for (int i = 0; i < segmentCount; i++)
            {
                double turn = HeadingDifference(headings[i], headings[(i + 1) % segmentCount]);
                if (turn > largestTurn)
                {
                    largestTurn = turn;
                    startSegment = (i + 1) % segmentCount;
                }
            }

            bool hasGroup = false;
            vec3 groupStart = new vec3();
            vec3 groupEnd = new vec3();
            double sinSum = 0;
            double cosSum = 0;

            for (int i = 0; i < segmentCount; i++)
            {
                int segmentIndex = (startSegment + i) % segmentCount;
                double heading = headings[segmentIndex];

                if (!hasGroup)
                {
                    hasGroup = true;
                    groupStart = segmentStarts[segmentIndex];
                    groupEnd = segmentEnds[segmentIndex];
                    sinSum = Math.Sin(heading);
                    cosSum = Math.Cos(heading);
                    continue;
                }

                double averageHeading = Math.Atan2(sinSum, cosSum);
                if (averageHeading < 0) averageHeading += glm.twoPI;

                if (HeadingDifference(averageHeading, heading) > sameSideAngleTolerance)
                {
                    sides.Add(Tuple.Create(groupStart, groupEnd));
        ...
        private List<vec3> CleanBoundaryPoints(List<vec3> fenceLine)
        {
            List<vec3> points = new List<vec3>();
            if (fenceLine == null) return points;

            foreach (vec3 point in fenceLine)
            {
                if (points.Count == 0 || BoundaryPointDistance(points[points.Count - 1], point) > 0.05)
                {
                    points.Add(point);
                }
            }

            if (points.Count > 1 && BoundaryPointDistance(points[0], points[points.Count - 1]) < 0.5)
            {
        ...
        private static double BoundaryPointDistance(vec3 pointA, vec3 pointB)
        {
            double deltaEast = pointB.easting - pointA.easting;
            double deltaNorth = pointB.northing - pointA.northing;
        ...
        private static double HeadingDifference(double headingA, double headingB)
        {
            double difference = Math.Abs(headingA - headingB);
            if (difference > Math.PI) difference = glm.twoPI - difference;
            return difference;
        }
        ...
        private void imuRollWizardMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormImuRollWizard"];
            ...
            Form form = new FormImuRollWizard(this);
            form.Show(this);
        }

        private void autoRollOffsetWizardMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormAutoRollOffsetWizard"];
            ...
            Form form = new FormAutoRollOffsetWizard(this);
            form.Show(this);
        }

        private void headingOffsetWizardMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormHeadingOffsetWizard"];
            ...
            Form form = new FormHeadingOffsetWizard(this);
            form.Show(this);
        }

        private void xteGuardWizardMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormXteGuardWizard"];
            ...
            Form form = new FormXteGuardWizard(this);
            form.Show(this);
        }

        private void rollCutMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormRollCutSettings"];
            ...
            Form form = new FormRollCutSettings(this);
            form.Show(this);
        }

        private void autoRollLearnMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormAutoRollLearnSettings"];
            ...
            Form form = new FormAutoRollLearnSettings(this);
            form.Show(this);
        }

        private void adaptiveSteeringMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormAdaptiveSteeringSettings"];
            ...
            Form form = new FormAdaptiveSteeringSettings(this);
            form.Show(this);
        }

        private void adaptiveSteeringAiMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormAdaptiveSteeringAi"];
            ...
            Form form = new FormAdaptiveSteeringAi(this);
            form.Show(this);
        }

        private void bodyLineHoldMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormBodyLineHoldSettings"];
            ...
            Form form = new FormBodyLineHoldSettings(this);
            form.Show(this);
        }

        private void obstacleOffsetMenuItem_Click(object sender, EventArgs e)
        {
            Form fc = Application.OpenForms["FormObstacleOffsetSettings"];
            ...
            Form form = new FormObstacleOffsetSettings(this);
            form.Show(this);
        }
