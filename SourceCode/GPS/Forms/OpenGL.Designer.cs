...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        private void DrawFlags()
        {
            ...
            try
            {
                foreach (CFlag flag in flagPts)
                {
                    ...
                    font.DrawText3D(flag.easting, flag.northing, flagColor + StripObstacleTag(flag.notes), camHeading);
                }
                if (flagNumberPicked != 0)
                {
        ...
        private void DrawBodyLineHoldAxis()
        {
            ...
            CTrk activeTrack = trk.gArr[trk.idx];
            double axisLength = glm.Distance(activeTrack.endPtA, activeTrack.endPtB);
            if (axisLength < 5.0)
            {
                axisLength = glm.Distance(activeTrack.ptA, activeTrack.ptB);
            }

            double heading = glm.toRadians(pn.headingTrueDual);
            double wheelbase = Math.Max(2.0, vehicle.VehicleConfig.Wheelbase);
            double halfLength = Math.Max(2.5, axisLength * 0.5);
            double centerLong = wheelbase * 0.5;

            double centerEasting = pivotAxlePos.easting + Math.Sin(heading) * centerLong;
            double centerNorthing = pivotAxlePos.northing + Math.Cos(heading) * centerLong;

            void AddPoint(double longitudinal, double lateral)
            {
                GL.Vertex3(
                    centerEasting + (Math.Sin(heading) * longitudinal) + (Math.Cos(heading) * lateral),
                    centerNorthing + (Math.Cos(heading) * longitudinal) - (Math.Sin(heading) * lateral),
                    0.0);
            }

            double arrowLength = Math.Min(0.85, halfLength * 0.3);
            double arrowWidth = Math.Min(0.45, halfLength * 0.16);

            GL.Disable(EnableCap.Texture2D);
            GL.LineWidth(4);
            GL.Color4(0.0f, 0.0f, 0.0f, 0.72f);
            GL.Begin(PrimitiveType.Lines);
            AddPoint(-halfLength, 0.0);
            AddPoint(halfLength, 0.0);
            AddPoint(halfLength, 0.0);
            AddPoint(halfLength - arrowLength, arrowWidth);
            AddPoint(halfLength, 0.0);
            AddPoint(halfLength - arrowLength, -arrowWidth);
            GL.End();

            GL.LineWidth(IsBodyLineHoldActing ? 2.0f : 1.5f);
            if (IsBodyLineHoldActing)
            {
                GL.Color4(1.0f, 0.86f, 0.05f, 0.96f);
            }
            else
            {
                GL.Color4(1.0f, 0.78f, 0.05f, 0.68f);
            }
