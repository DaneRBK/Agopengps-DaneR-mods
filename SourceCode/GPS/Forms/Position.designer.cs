...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        public double rollCorrectionEasting = 0;
        public double rollCorrectionNorthing = 0;
        ...
        public void UpdateFixPosition()
        {
            ...
            startCounter++;
            rollCorrectionEasting = 0;
            rollCorrectionNorthing = 0;
                ...
                case "Fix":
                    {
                        ...
                        if (!isFirstHeadingSet) //set in steer settings, Stanley
                        {
                            ...
                            if (stepFixPts[2].isSet == 0)
                            {
                                ...
                                if (ahrs.imuRoll != 88888)
                                {
                                    ...
                                    for (int i = 0; i < 3; i++)
                                    {
                                        rollCorrectionEasting = Math.Cos(-gpsHeading) * rollCorrectionDistance;
                                        rollCorrectionNorthing = Math.Sin(-gpsHeading) * rollCorrectionDistance;
                                        stepFixPts[i].easting = rollCorrectionEasting + stepFixPts[i].easting;
                                        stepFixPts[i].northing = rollCorrectionNorthing + stepFixPts[i].northing;
                                    }
                                }
                        ...
                        if (ahrs.imuRoll != 88888)
                        {
                            ...
                            pn.fix.easting = rollCorrectionEasting + pn.fix.easting;
                            pn.fix.northing = rollCorrectionNorthing + pn.fix.northing;
                        }
                ...
                case "VTG":
                    {
                        ...
                        if (ahrs.imuRoll != 88888)
                        {
                            ...
                            rollCorrectionDistance = Math.Sin(glm.toRadians((ahrs.imuRoll))) * -vehicle.VehicleConfig.AntennaHeight;
                            correctionDistanceGraph = rollCorrectionDistance;
                            rollCorrectionEasting = Math.Cos(-fixHeading) * rollCorrectionDistance;
                            rollCorrectionNorthing = Math.Sin(-fixHeading) * rollCorrectionDistance;
            ...
            {
                bool isMobileRemoteManualSteerActive = IsMobileRemoteManualSteerActive();
                ...
                if (isMobileRemoteManualSteerActive)
                {
                    p_254.pgn[p_254.speedHi] = unchecked((byte)((int)80 >> 8));
                    p_254.pgn[p_254.speedLo] = unchecked((byte)80);
                }
                ...
                if (!isBtnAutoSteerOn && !isMobileRemoteManualSteerActive) //32020 means auto steer is off
                {
                ...
                if (!timerSim.Enabled)
                {
                    if (isMobileRemoteManualSteerActive && avgSpeed > vehicle.maxSteerSpeed)
                    {
                        DisableMobileRemoteManualSteer();
                        isMobileRemoteManualSteerActive = false;
                        p_254.pgn[p_254.status] = 0;
                    }
                    ...
                    if (!isMobileRemoteManualSteerActive && vehicle.minSteerSpeed >= 0.1)
                    {
                ...
                if (isMobileRemoteManualSteerActive)
                {
                    ...
                    p_254.pgn[p_254.lineDistance] = 127;
                    vehicle.deadZoneDelayCounter = 0;
                    vehicle.isInDeadZone = false;
                }

                if (!isMobileRemoteManualSteerActive && p_254.pgn[p_254.status] == 1 && !isReverse
                ...
                {
                    short steerCommand = isMobileRemoteManualSteerActive
                        ? GetMobileRemoteManualSteerCommand()
                        : ApplyXteGuard(guidanceLineSteerAngle);

                    if (!isMobileRemoteManualSteerActive)
                    {
                        steerCommand = ApplyBodyLineHold(steerCommand);
                        steerCommand = ApplyAdaptiveSteeringResponse(steerCommand);
                        UpdateAdaptiveSteeringAi(steerCommand);
                    }

                    p_254.pgn[p_254.steerAngleHi] = unchecked((byte)(steerCommand >> 8));
                    p_254.pgn[p_254.steerAngleLo] = unchecked((byte)(steerCommand));

                    // Smart WAS sample collection - guidance steer angle in degrees
                    if (!isMobileRemoteManualSteerActive) smartWAS.AddSample(guidanceLineSteerAngle * 0.01);
                }
            }
                ...
                else //Youturn is on
                {
                    ...
                    if (isInTurnBounds)
                    {
                        ...
                        if (yt.youTurnPhase != 10)
                        {
                            ...
                            {
                                bool isABTurn = trk.idx >= 0 && trk.idx < trk.gArr.Count && trk.gArr[trk.idx].mode == TrackMode.AB;
                                if (!yt.TrySwitchToOppositeSideAfterFailure(isABTurn))
                                {
        ...
        private void TheRest()
        {
            ...
            double sectionToolEasting = GetGpsOnlyMappingToolEasting();
            double sectionToolNorthing = GetGpsOnlyMappingToolNorthing();
            double sectionFixEasting = GetGpsOnlyMappingFixEasting();
            double sectionFixNorthing = GetGpsOnlyMappingFixNorthing();

            CalculateSectionLookAhead(sectionToolNorthing, sectionToolEasting, cosSectionHeading, sinSectionHeading);

            //To prevent drawing high numbers of triangles, determine and test before drawing vertex
            sectionTriggerDistance = glm.Distance(sectionFixEasting, sectionFixNorthing, prevSectionPos.easting, prevSectionPos.northing);
        ...
        public double GetGpsOnlyMappingToolEasting()
        {
            return toolPos.easting - rollCorrectionEasting;
        }

        public double GetGpsOnlyMappingToolNorthing()
        {
            return toolPos.northing - rollCorrectionNorthing;
        }

        public double GetGpsOnlyMappingFixEasting()
        {
            return pn.fix.easting - rollCorrectionEasting;
        }

        public double GetGpsOnlyMappingFixNorthing()
        {
            return pn.fix.northing - rollCorrectionNorthing;
        }
        ...
        private void AddSectionOrPathPoints()
        {
            ...
            //save the north & east as previous
            prevSectionPos.northing = GetGpsOnlyMappingFixNorthing();
            prevSectionPos.easting = GetGpsOnlyMappingFixEasting();
