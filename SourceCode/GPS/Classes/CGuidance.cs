    ...
    public class CGuidance
    {
        ...
        private void DoSteerAngleCalc()
        {
            ...
            if (rollForSteering != 88888)
                steerAngleGu += rollForSteering * -sideHillCompFactor;
        ...
        {
            double obstacleOffset = mf.ABLine.GetObstacleAvoidanceOffsetMeters(pivot, mf.vehicle.UpdateGoalPointDistance());
            if (Math.Abs(obstacleOffset) > 0.01)
            {
                vec2 right = mf.ABLine.GetTravelRightUnitVector();
                curPtA.easting += right.easting * obstacleOffset;
                curPtA.northing += right.northing * obstacleOffset;
                curPtB.easting += right.easting * obstacleOffset;
                curPtB.northing += right.northing * obstacleOffset;
            }
