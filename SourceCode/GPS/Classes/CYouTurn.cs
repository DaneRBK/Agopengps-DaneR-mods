...
namespace AgOpenGPS
{
    ...
    public class CYouTurn
    {
        ...
        private bool hasTriedOppositeSideAfterFailure;
        #endregion
        ...
        private bool IsLaneIncreasing(bool turnLeft, bool isAB)
        {
            if (isAB)
                return (turnLeft && !mf.ABLine.isHeadingSameWay) || (!turnLeft && mf.ABLine.isHeadingSameWay);

            return (turnLeft && !mf.curve.isHeadingSameWay) || (!turnLeft && mf.curve.isHeadingSameWay);
        }

        private int GetCandidateLane(bool turnLeft, int rowSkipsWidth, bool isAB)
        {
            int currentLane = isAB ? mf.ABLine.howManyPathsAway : mf.curve.howManyPathsAway;
            return currentLane + (IsLaneIncreasing(turnLeft, isAB) ? rowSkipsWidth : -rowSkipsWidth);
        }

        private int FindNextSelectableTrack(bool turnLeft, int rowSkipsWidth, bool isAB, out bool found)
        {
            rowSkipsWidth = Math.Max(1, rowSkipsWidth);

            for (int searchLimit = 0; searchLimit < 200; searchLimit++)
            {
                int lane = GetCandidateLane(turnLeft, rowSkipsWidth, isAB);
                if (mf.IsWorkedTrackSelectable(lane, isAB))
                {
                    found = true;
        ...
        {
            int selectedRowSkipsWidth = FindNextSelectableTrack(isTurnLeft, rowSkipsWidth, isAB, out bool found);
            if (found) return selectedRowSkipsWidth;

            int oppositeRowSkipsWidth = FindNextSelectableTrack(!isTurnLeft, rowSkipsWidth, isAB, out bool foundOpposite);
            if (foundOpposite)
            {
                this.isTurnLeft = !isTurnLeft;
                return oppositeRowSkipsWidth;
            }

            return rowSkipsWidth;
        }

        public bool TrySwitchToOppositeSideAfterFailure(bool isAB)
        {
            if (hasTriedOppositeSideAfterFailure) return false;

            int baseRowSkipsWidth = skipMode == SkipMode.Alternative ? rowSkipsWidth : Properties.Settings.Default.set_youSkipWidth;
            int oppositeRowSkipsWidth = FindNextSelectableTrack(!isTurnLeft, Math.Max(1, baseRowSkipsWidth), isAB, out bool foundOpposite);
            if (!foundOpposite) return false;

            hasTriedOppositeSideAfterFailure = true;
            isTurnLeft = !isTurnLeft;
            rowSkipsWidth = oppositeRowSkipsWidth;
            ResetCreatedYouTurn();
        ...
        {
            mf.MarkCurrentTrackWorkedIfComplete();
            int baseRowSkipsWidth = skipMode == SkipMode.Alternative ? rowSkipsWidth : Properties.Settings.Default.set_youSkipWidth;
            rowSkipsWidth = GetNextNotWorkedTrack(isTurnLeft, Math.Max(1, baseRowSkipsWidth), false);
        ...
        public bool BuildABLineDubinsYouTurn()
        {
            mf.MarkCurrentTrackWorkedIfComplete();
            int baseRowSkipsWidth = skipMode == SkipMode.Alternative ? rowSkipsWidth : Properties.Settings.Default.set_youSkipWidth;
            rowSkipsWidth = GetNextNotWorkedTrack(isTurnLeft, Math.Max(1, baseRowSkipsWidth), true);
        ...
        public void CompleteYouTurn()
        {
            ...
            hasTriedOppositeSideAfterFailure = false;
            ResetCreatedYouTurn();
            mf.sounds.isBoundAlarming = false;
        }
